...
mod tests {
    ...
    #[test]
    fn a_call_may_take_more_than_255_arguments() {
        // A minified bundle really does hand a helper a table in one go. The
        // count was a byte, so the whole file failed to compile.
        let result = run_document_scripts(
            &format!(
                "<html><body><script>document.title = String(Math.max({}));</script></body></html>",
        ...
        assert!(result.error.is_none(), "error: {:?}", result.error);
        assert_eq!(result.title.as_deref(), Some("299"));
    }
