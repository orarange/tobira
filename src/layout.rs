...
pub fn layout_styled_document(document: &StyledNode, viewport_width: u32) -> LayoutDocument {
    ...
    layout_node(document, 0, viewport_width, &mut cursor_y, &mut context);
...
impl LineBuilder {
    ...
    fn push_span(&mut self, text: &str, style: &ComputedStyle) {
        if text.is_empty() {
            return;
        }
...
fn layout_node(
    ...
    match node {
        StyledNode::Text(text) => {
            ...
            layout_inline_fragments(&fragments, &text.style, x, width, cursor_y, context);
        }
        StyledNode::Element(element) => match element.style.display {
            Display::None => {}
            Display::Inline => {
                let fragments = flatten_inline_fragments(node);
                layout_inline_fragments(&fragments, &element.style, x, width, cursor_y, context);
            }
            Display::Block | Display::ListItem => {
                layout_block_element(element, x, width, cursor_y, context);
            }
        },
    }
}
...
fn layout_block_element(
    ...
    if element.tag_name == "br" {
        *cursor_y = cursor_y.saturating_add(text_line_height(&element.style));
        return;
    }
...
fn layout_mixed_children(
    ...
    for child in &element.children {
        ...
        if is_block_level(child) {
            ...
            layout_node(child, x, width, cursor_y, context);
        } else {
            if bullet_pending {
...
fn layout_inline_fragments(
    ...
    if container_style.white_space == WhiteSpaceMode::Pre {
        layout_preformatted_fragments(fragments, container_style, x, width, cursor_y, context);
    } else {
        layout_normal_fragments(fragments, container_style, x, width, cursor_y, context);
    }
}
        ...
        match fragment {
            ...
            InlineFragment::Text { text, style } => {
                for character in text.chars() {
                    ...
                    line.push_span(character.encode_utf8(&mut buffer), style);
                }
            }
        }
    }
...
fn emit_line(
    ...
    if line.is_empty() {
        *cursor_y = cursor_y.saturating_add(text_line_height(container_style));
        return;
    }
    ...
    let line_height = line.line_height.max(text_line_height(container_style));
...
mod tests {
    ...
    fn hides_display_none_content() {
        ...
        let layout = layout_styled_document(&styled, 320);
    ...
    fn centers_text_when_requested() {
        ...
        let styled = build_styled_tree(&document, &stylesheet);
        let layout = layout_styled_document(&styled, 200);
    ...
    fn wraps_text_across_multiple_lines() {
        ...
        let styled = build_styled_tree(&document, &stylesheet);
        let layout = layout_styled_document(&styled, 90);
