//! The HTML standard's named character references.
//!
//! Generated from `https://html.spec.whatwg.org/entities.json`.

/// Every named character reference in the HTML standard, as published in
/// `entities.json`.
///
/// The table used to hold six names. Pages lean on hundreds -- `&mdash;`,
/// `&rsquo;`, `&hellip;`, `&times;` -- and an unknown one was left on screen as
/// its own source text.
///
/// Names are stored without the leading `&`. The ones that keep a trailing `;`
/// require it; the 106 without are the legacy set a browser still resolves when
/// the semicolon is missing. Sorted, so a lookup is a binary search.
...
/// The longest named reference matching the start of `rest`, and its
/// replacement.
///
/// HTML matches the *longest* name, and a name may end without its semicolon:
/// `&notit;` is `¬it;`, because `not` matches and `notin;` does not. The table
/// is sorted, so the candidates are a contiguous run.
pub fn longest_match(rest: &str) -> Option<(usize, &'static str)> {
    let mut best: Option<(usize, &'static str)> = None;
    for (name, replacement) in NAMED_ENTITIES {
        if rest.len() >= name.len() && rest.as_bytes()[..name.len()] == *name.as_bytes() {
...
/// Whether the name needs its semicolon. The legacy set that does not is the
/// only reason a reference can end without one.
pub fn requires_semicolon(name: &str) -> bool {
    !NAMED_ENTITIES
