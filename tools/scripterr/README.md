...
| `nativesymbol.html` | the checks core-js uses to decide that Symbol and `Object.assign` are native-grade | one line still differs: the `assign` getter case (`String(Symbol("x"))` and `Object(Symbol()) instanceof Symbol` were fixed on 2026-09-19). `tools/polyfill/replaced.html` counts what that costs |
