...
- Next campaign targets (deeper, left for interactive work): getbootstrap `bootstrap is not defined` (astro app code); dynamic `import()` (crates.io, svelte.dev — mid-size ESM feature with scope choices); vitejs.dev genuine infinite recursion; webpack/next `modules[id].call` (react.dev, typescriptlang). CLEAN: example, HN, Wikipedia, web.dev, vuejs, rust-lang, docs.rs, lodash, expressjs.
