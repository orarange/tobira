...
- **rollupjs.org `["items"]` PINPOINTED with the new backtrace.** It reads `at <script> (2:40706)`. `theme.DwJmkNlp.js` line 2 col 40706 is `const vs=_o(); … const ys=vs((e,t,n,o,s,i)=>(l(),P("div",gs)));` — the failing call is `vs(arrowFn)`. So `vs = _o()` returned the array `["items"]` instead of a function. `_o` is an ESM import: the import statement is `import{… _ as L, … L as _o, …}from"./framework.P2XOc7lE.js"` — note the collision, export `_`→local `L` AND export `L`→local `_o`. **Strong hypothesis: named-import binding resolution picks the wrong export when export names collide with local alias names** (so `_o` binds to something whose value is `["items"]`, or `_o()`'s result is wrong). NEXT STEP: reproduce ESM `import {a as b, b as c}` alias-collision binding in a unit test against our module loader (`src/engine/compiler/modules.rs` + the host module graph in `engine_host.rs`); if binding is correct, inspect what framework export `L` is and why `L()` yields `["items"]`.
