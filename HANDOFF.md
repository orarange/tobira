...
2026-09-10 の夜に前の一覧（1〜10）は一巡した。片付いたもの、測って
**やらんと決めた**もの、その晩に新しく見つかったものを混ぜて並べ直してある。
各項目の数字は「今こうなっとる」という実測。

1. **`font-size` の端数** — `LengthValue` が u32 なので `10pt`（13.333px）が
   13px、`11pt`（14.667px）が 15px になる。**これが今いちばん大きい単独の穴。**
   影響が二方向に出る:
   - 字幅: `tools/geom/fsize.html` が 3/12。**整数の大きさは全部合うようになった**
     ので、残る 9 件は全部これ。
   - 行の高さ: `tools/geom/overflow.html` の残り 7 件も全部これ
     （`10pt` で組んだ頁なので語が 328 やのうて 320、表の行が 22 やのうて 18）。

   **規模を測った上で 2026-09-10 には着手せんかった。** `font_size_px` の
   読みが 141 箇所、測定 API の呼び出しが 48 箇所あって、一つ一つ
   「箱の勘定に使う整数か、測定に使う実寸か」の判断が要る。単位の変更が
   中途半端な状態が一番危ない。

   **やるなら設計を先に決めること。** 案としては、`ComputedStyle` が
   百分の一 px を持ち、`font_size_px()` を丸めた値を返すメソッドにして、
   font.rs の公開関数（`text_width_px` / `line_height_px` /
   `content_height_px` / `glyph_advance_px` / `draw_text`）は百分の一 px を
   取る。**描画と測定が同じ値を使うこと**が条件（違うと字が箱からはみ出す）。

2. **インライン矩形の残り（g2 / g4）** — `g4` が 11/14、`g2` が 14/22。
   残るのは (a) 空白だけの div の中の空 `<span>`、(b) 行をまたぐ `<span>`、
   (c) `<a>` の x が 3px ずれる。**2026-09-10 に三度直そうとして三度悪化させた**
   （10/14 → 6〜8/14）。原因は `emit_line_impl` で閉じ印が自分の run より先に
   処理される構造で、その順序に他の計算がぶら下がっとる。**部分的に触ると
   壊れる。走査順を設計からやり直すこと。**
   詳しくは `tools/geom/README.md` の sup.html の節。
   `sup.html` が 2/10 なのも同じ根（インライン要素が自分の字やのうて
   親の字の高さを返す）。

3. **幾何の配線を広げる** — 版面は要素ごとに、エンジンに渡しとる
   5 つ組 `(node_id, x, y, w, h)` より多くを知っとる。そのせいで
   **DOM 側が答えられん質問が三つ**残っとる:
   - `scrollWidth` / `scrollHeight` が箱の寸法を返す（本当は中身の広がり）。
     `tools/geom/overflow2.html` が 6/12 で、外れは全部これ。
   - `clientWidth` / `clientHeight` が巻物の分を引かん。
     `tools/geom/scrollbar.html` が 3/7 で、**箱の寸法は七形とも一致した**。
     外れは全部これ。
   - `Range.getBoundingClientRect` が無い（`fsize.html` を最初その形で
     書いて全行 0 が返った。span で測り直してある）。
   三つとも「版面が持っとる数字をエンジンまで運ぶ」一つの仕事に帰着する。

4. **縦の巻物の場所** — 横は入った（`1fa56de`）。縦は
   `overflow: scroll` なら無条件なので**子を配置する前に幅から引ける**が、
   `auto` は「縦にはみ出すか」を先に知らなあかんので二段レイアウトが要る。
   Chrome の実測は `tools/geom/scrollbar.html` に入っとる（a5 の `cw=105`）。

5. **`overflow-x` と `overflow-y` が一つに潰れとる** — 「より厳しいほう」を
   採る実装。`overflow-x: hidden` だけ書いた頁で縦も clip される。
   **ただし `tools/geom/overflow2.html` で測った限り版面には出とらん**
   （箱は六形とも Chrome と一致）。直すなら軸ごとに持つ必要があって、
   clip の判定が 8 箇所ある。**実害を先に見つけてからやること。**

6. **html5lib 残り 19 件** — 98.5%。塊は `<font>` の入れ子（tests23 に 3、
   養子縁組の深いところ）と `<col>`/`<colgroup>` の表構造（tests1 に 2）。
   後者は過去に 1184 → 1115 の退化を出した領域と地続き。
   残りは文書の頭の空白（doctype01・tests15・tests19 に各 1〜2）と、
   表の中のフォームの里親付け（html5test-com）。
   **`<isindex>` の 9 件は片付いた**（`e088cc0`）。「追わん」と決めとった
   判断を引っくり返した理由はそのコミットに書いてある。

7. **`Map` / `Set` の索引化** — 走査は今も線形で、**二乗であることは
   測って確かめた**（n を倍にすると時間が約 3.7 倍。素のオブジェクトは
   きっちり 2 倍。n=4000 で Map 67ms 対オブジェクト 8ms）。
   **やらんと決めた。** 索引を安全に持つには `ObjectKind` の形を変える
   必要があって 43 箇所に及ぶ。GC のときと同じで「登録漏れが起きたら
   気づける仕掛け」が要る領域。独立した回でやること。
   測定は `map.set` / `map.set+get` / `set.add` / `set.add+has` を
   n=250〜4000 で回すだけ（`tests/` に一時ファイルとして書いて消した）。

8. **楕円の角** — `border-radius: 10px / 30px` の縦横で違う半径。
   角ごとの半径は入った（`97498fe`）が、角は真円一つしか持っとらん。
   `radius2.html` の r4 と `radius.html` の r5 がこれで外れとる。

9. **文字列が符号点で数えられとる** — `'😀'.length` が 1（JS は 2）、
   `charCodeAt(0)` が 128512（JS は 55357）。**一貫はしとる**ので
   `s[i]` の往復は壊れとらんが、JS の 16bit 単位モデルとは別物。
   surrogate を触る polyfill や絵文字の勘定が合わん。
   `.length` だけ直すと `for (i=0;i<s.length;i++) s[i]` が範囲外に出るので、
   **直すなら添字も全部まとめて**。

10. **総称 `monospace` / `serif` の当て先** — Chrome headless は
   monospace で 120（MS Gothic / MS Mincho / Tahoma のどれか）、
   serif で 137（Noto 系）。tobira は Consolas 132 / Georgia 124。
   **当てれば照合の数字は上がるが、やらんかった。** Consolas のほうが
   code の見た目として明らかにええし、数字のために見た目を悪うする
   判断はしたない。`cursive` / `fantasy` は顔の別名やったので当てた
   （`111b88e`）。
...
### 2026-09-10 - Claude (自走ループ二晩目: 8535535..bc8bbfd, 15 コミット)

一晩目と同じ「次の一手を上から順に、09:00 まで自分で回せ」。一覧を
一巡した。数字は 1154 → **1172 通過**、html5lib 97.3% → **98.5%**。
