...
- Verification status:
- `cargo test`: `661` passing tests on `2026-07-22` (Windows checkout)
...
- **Next lead — `Reflect.construct` ignores its newTarget argument, and `new.target` is not propagated.** This is pre-existing and NOT specific to DOM interfaces: it reproduces on ordinary functions (`Object.getPrototypeOf(Reflect.construct(Base, [], Derived)) === Derived.prototype` is `false`; `new.target` inside a constructor invoked via `Reflect.construct` is not the passed newTarget). Babel's `_createSuper` is built on exactly this, so every transpiled `class extends` currently gets the wrong instance prototype — likely the widest-reaching remaining JS gap and the best next target.
