...
- 580 tests green. Diagnostic sweep intel for next targets: dynamic `import()` is the most common remaining gap (crates.io, svelte.dev) — a mid-size ESM feature with scope choices, better tackled interactively; `modules[id].call` webpack/next runtime (react.dev, typescriptlang); vitejs.dev infinite recursion (deep); docs.rs is CLEAN.
