...
- Real-page campaign: implemented the `URL` global (hand-rolled parser, no new dep) — nodejs.org and tailwindcss.com stopped on `URL is not defined`; both now advance (nodejs.org's next wall is `getAttribute is not a function`). 578 tests green; new `tests/url_global.rs`. Diagnostic sweep intel for next targets: dynamic `import()` is the most common remaining gap (crates.io, svelte.dev); `modules[id].call` webpack/next runtime (react.dev, typescriptlang); vitejs.dev hits a phase-3 call-stack-depth limit (worth investigating — may be a clean fix); docs.rs is CLEAN.
