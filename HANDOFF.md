...
- Verification status:
- `cargo test`: `652` passing tests on `2026-07-22` (Windows checkout)
...
- Still open from the earlier probe: `typeof HTMLElement` / `typeof EventTarget` report `"object"` because `DOM_INTERFACE_NAMES` globals are built with `allocate_ordinary_object` (no `.name`, no `.prototype.constructor`). Suspected cause of the rollupjs.org `object is not callable` lead — transpiled `class X extends HTMLElement` calls `Reflect.construct(HTMLElement, …)`, which hits `require_callable` (`vm.rs`). Native `class extends` already works, so a source-syntax probe does not reproduce it. Also unfixed: arrow functions and shorthand/class methods still have a `.prototype` (spec says they must not).
