...
- **省リソースは最大の目標**（2026-09-20、minato の訂正）。ただし
  **ブラウザとして実際に機能するものでないとアカン**。順位はこう:
  ① 機能すること（正しさ）= 譲れん下限 → ② 省リソース = **最大の目標** →
  ③ 機能の広さ。
  08-23 に「まず実用ブラウザ」へ振ったときの**後半（正しさを落とす判断は
  もうしとらん）は生きとる**が、「第二目標」という順位付けは**誤り**やった。
  この行は 09-20 まで「第二目標」と書いてあって、`<isindex>` と同じドリフト
  になっとった。
  **2026-09-20 に計器ができた**（`tools/resource/measure.py`）。それまで
  正しさの計器は五本あったのに、**資源の計器は一本も無かった**。
  Chrome は多プロセスなので**プロセス木を全部足して**比べる（親だけやと
  tobira が不当に重く見える。「両方に同じ質問をしたか」）。
  - 初回（六枚、1280x900）: **メモリは問題やない。CPU が問題**。
    tobira は六枚中五枚で Chrome より軽く、script の無い HN では **10%**
    （50.7 MiB 対 510.5）。ただし **react.dev が CPU 38.6 秒 対 Chrome 2.6 秒、
    wall は 59 秒 対 1 秒**。ja.wikipedia だけメモリが Chrome を超える（106%）。
  - **CPU の行き先は settle ループ**と判明: `TOBIRA_SETTLE_MS=0` なら
    6.06 秒、既定の 2000 で 38.05 秒。**32 秒が settle**。
    `TOBIRA_DYNAMIC_SCRIPTS=0` でも 37.7 秒なので動的 script やない。
    settle は毎フレーム文書全体を組み直しとる（09-18 の実装）ところ、
    react.dev は 1846 要素。`layout_dirty` / `ensure_layout` の
  ...
  - **どこに消えとるかを測った（2026-09-20）**: 予算を振ると
    ...
    **一次式で、1 フィード = 0.93 CPU 秒**（切片 5.6 秒が素の起動）。
    125 フレーム中 layout は 35 回だけ（`tick` は既に「DOM が変わった時だけ」
    true を返しとる）。
  - **「layout 一回が 0.93 秒」は誤りやった**（同日、`TOBIRA_TIME_LAYOUT=1`
    で実測して訂正）。内訳はこれ:
    ```
    tick(changed=true)  n=34  計 48.9 s  平均 1437 ms   ← ここ
    tick(changed=false) n=91  計  0.1 s  平均    1 ms
    layout_styled_document n=35 計 1.5 s  平均   44 ms
    ```
    **layout は 44ms しかかかっとらん。食うとるのは `page.tick()` の中、
  ...
  - **その 1.4 秒の正体も割った**（同日、`TOBIRA_TIME_LAYOUT=1`）。
    ...
    **次の的はここ**: 属性・テキストの変更も増分で当てられるようにする
    ...
    - **【訂正】「犯人は styling」は外れの公算**（同日、Mac の指摘）。
      **合成頁 683ms と実頁 1402ms は 2 倍違う。再現できとらんかった。**
      react.dev の実 CSS は 106KB / 規則 1511 本 / **バケツ 712 個**、
      最大でも universal の 120 本（7.9%）で**鍵は散っとる側**。
      1847 要素 × 候補 150 本 × 316ns ≒ **87ms** で、実測と **16 倍合わん**。
      散った形の合成頁でも styling は +9ms しか足さんかった。
      **次は合成頁やのうて react.dev そのもので**: ① `candidates_for` の
      候補数の分布を数える ② 掛け算が 1402ms を説明するか
      ...
      - **【当たり】固定費の正体は日本語フォントの読み込み**（2026-09-20、
        ...
        **日本語五文字で 243 MiB と 1.0 CPU 秒。** 検体
        ...
        - **二つに割れた**（同日、字種を振って実測）:
          ```
          日本語 1 文字      274.7 MiB   0.94 CPU s
          日本語 800 文字    485.8 MiB   2.05 CPU s
          ```
          ① **最初の一文字で 275 MiB** — フォント本体の読み込みと展開。
          `msgothic.ttc` が 8MB、`YuGothM.ttc` が 13MB なので、
          **40 倍展開なら 320MB。桁が合う。**
          ② **【訂正】「一字 0.26 MiB」は誤り。glyph cache は無罪。**
          字数を振ったら**直線やのうて段**やった:
          ```
             1 字  274.8 MiB      100 字  486.9 MiB
            10 字  486.2 MiB      400 字  486.6 MiB
                                  800 字  486.2 MiB
                                 1600 字  485.7 MiB   ← 以降ずっと平ら
          ```
          **1600 字が 10 字と同じ。** つまり 1〜10 字の間で
          **二つ目のフォント本体を読んどる**（+211 MiB）だけで、
          **glyph cache は 1600 字積んでも増えん**。
          `font_size_mpx` を鍵にして値域が 10000 倍密になった件
          （固定小数の副作用）も、**ここでは効いとらん**。
          前便で「一字 0.26 MiB」と書いたのは、1 字と 800 字の二点だけを
          見て傾きを引いたから。**二点で線を引いたら段を見逃す。**
          → **犯人はフォント本体の数**。`named_fonts:
          HashMap<(u16,bool), Vec<Font>>` に、字ごとの fallback で本体が
          積み上がる。一本 200〜300 MiB（40 倍展開）なので、**二本で 486**。
          → **一時的やのうて居座る分が主**。`bytes.clone()` × 4 は 32MB
          程度で、275 MiB の説明にはならん。**①②（clone と 4 回試行）は
          CPU には効くがメモリにはほぼ効かん。③（上限と追い出し）が本命。**
    ...
    - 以下は**その外れた経緯**（合成頁で実測）。
      1800 要素・属性だけを書き換えるタイマー・apply 33 回を揃えて、
      **CSS 規則 1200 本あり = 683 ms / 回、規則ゼロ = 15 ms / 回**。
    ...
    - 測り方の教訓: **保存した頁では測れん**。react.dev の保存 HTML は
      ...
      **同じ形（属性だけ変わる・子リストは変わらん）の合成頁を作る**方が早い。
...
- `git add -A` は使わん。`git add -u` か、パスを明示。
  ...
  **規律やのうて手順で止める**: `ship.sh` が **初めて追加されるファイルを
  ...
  untracked 検査に続く四つ目。
- **最小再現は、まず数字を再現せなアカン。掛け算が合うか毎回見る。**
  2026-09-20、合成頁の 683ms を react.dev の 1402ms の再現やと二人とも
  思い込んで、その上に「316ns/照合」「索引の設計」まで積んだ。**2 倍違う**
  のを誰も見んかった。**合わん掛け算はその日三回通っとる**
  （0.93秒＝layout、683＝1402 の再現、316ns×索引なし）。
  **合うまでは誰も犯人やない。**
- **相関は測れても、因果は測れとらん。傾きに名前を付けるときにサボらん。**
  2026-09-20 に react.dev の CPU を追うたとき、予算を振って
  「**1 フィード = 0.93 CPU 秒**」という一次式まで出した。そこまでは正しい。
  外したのは**その 0.93 秒に「layout」と名前を付けた**所で、実際は
  layout は 44ms、食うとったのは隣で走る `apply_script_snapshot`（1.4 秒）
  やった。さらにその 1.4 秒も、割ったら「増分経路が 34 回中 33 回
  断られとる」という別の形やった。**曲線を引いたら、傾きの中身も割る**まで
  やって初めて答えになる。`TOBIRA_TIME_LAYOUT=1` がその道具。
...
- ついでに出た本物のバグ: **`el.style.cssText = "..."` が `css-text` という
  プロパティ名として扱われ、`style="css-text: height:40px"` になって何も
...
- **次に手を付ける二つ（`layout.rs`、Mac が読んで見つけた）**。どちらも
  ...
  - ① **`line_height_from_ratio` が整数 px に丸めとる**（7220 付近）。
    ...
    **やってみた（2026-09-20）。位置は直るが箱が膨らむので戻した。**
    ...
    `<br>` 10 行の塊）で A/B:
    ```
    債務なし: p20 が y=342（Chrome 349、7px 手前）、塊の高さ 180（Chrome 184）
    債務あり: p20 が y=349（一致）、塊も一致に近付く
              ただし各 <p> の高さが 18 → 19（Chrome は 18）
    ```
    **この実装では「行の advance」と「箱の高さ」が同じ一つの数**なので、
    繰り越した 1px が箱の高さにも乗る。Chrome は内部を 1/64px で持って、
    箱は 18.4 のまま `getBoundingClientRect` で 18 に丸め、位置は正確に
